...
/*
RpcProvider 是 RPC 框架的服务端实现，核心职责：
1. 注册（发布）本地提供的 RPC 服务和方法；
2. 启动网络服务，监听指定 IP / 端口，处理客户端连接；
3. 解析客户端发来的 RPC 请求（基于 Protobuf 序列化）；
4. 调用本地注册的业务方法，将执行结果序列化后返回给客户端；
5. 记录服务节点的 IP / 端口到配置文件，方便服务发现。
*/
